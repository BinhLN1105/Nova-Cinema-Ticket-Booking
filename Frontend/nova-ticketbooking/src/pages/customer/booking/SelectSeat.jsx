...
export default function SelectSeatPage() {
  ...
  const { selectedSeats, toggleSeat, selectedShowtime, total } =
    useBookingStore();
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <div>
            ...
            {selectedShowtime && (
              <p className="text-cinema-300 text-sm">
                {selectedShowtime.cinemaName} • {selectedShowtime.screenName}
              </p>
            )}
          </div>
        </div>
              ...
              <div className="space-y-2">
                {Object.entries(rows).map(([rowLabel, seats]) => (
                  <div key={rowLabel} className="flex items-center gap-2">
                    <span className="w-6 text-cinema-500 text-xs font-bold text-right">
                      {rowLabel}
                    </span>
        ...
        <div className="flex flex-wrap gap-6 mt-8 justify-center text-xs text-cinema-300">
          {[
            ...
            {
              color: "bg-purple-500/20 border border-purple-500/40",
              label: "Ghế VIP",
            },
            {
              color: "bg-gold-400/20 border border-gold-400/40",
              label: "Ghế đôi",
            },
            {
              color: "bg-brand-500 border border-brand-400",
              label: "Đang chọn",
            },
            {
              color: "bg-cinema-700 border border-cinema-600 opacity-40",
              label: "Đã đặt",
            },
          ].map(({ color, label }) => (
