  ...
  return (
    <div className="bg-cinema-900 min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-24">
        ...
        >
          {/* Header */}
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4 border-b border-white/5 pb-6">
            <div>
              <span className="font-mono text-xs tracking-[0.25em] text-[#F5C518] uppercase">
                ── [ SPECIAL PRIVILEGES ] ──
              </span>
              <h2 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-white tracking-tight mt-2.5">
                Đặc Quyền Điện Ảnh <span className="text-white">Nova<span className="text-[#F5C518]">Ticket</span></span>
              </h2>
            </div>
            <p className="text-cinema-300 text-sm max-w-md font-body leading-relaxed md:text-right">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className={cn(
              "md:col-span-2 relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#121e2f] via-[#0d1724] to-[#070b12] p-8 sm:p-10 shadow-2xl flex flex-col justify-between group border transition-all duration-300",
              currentTier.heroBorder
            )}>
              {/* Background ambient lighting */}
              <div className={cn("absolute top-0 right-0 w-96 h-96 bg-gradient-to-bl blur-3xl pointer-events-none", currentTier.heroGlow)} />
              ...
              <div className="relative z-10 flex flex-col sm:flex-row sm:items-start justify-between gap-6 mb-8">
                <div className="max-w-md">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="font-mono text-xs text-[#F5C518] font-extrabold tracking-wider">FEATURED REEL</span>
                    <span className="w-1.5 h-1.5 rounded-full bg-[#F5C518] animate-pulse" />
                    <span className="text-xs font-mono font-bold text-amber-300 tracking-wider">01 / CINEPOINT VIP</span>
                  </div>
                  <h3 className="font-display text-2xl sm:text-3xl font-bold text-white mb-3 group-hover:text-[#F5C518] transition-colors leading-tight">
                    Hội Viên VIP & Tích Lũy Điểm CinePoint
                  </h3>
                  <p className="text-cinema-100 text-sm leading-relaxed font-body">
                    {isAuthenticated ? (
                      <>
                        Chào <strong className="text-white font-bold">{user?.fullName || 'Hội viên'}</strong>! Hạng thẻ <strong className={cn("font-bold", currentTier.titleColor)}>{user?.membershipTier || 'BRONZE'}</strong> của bạn nhận hoàn <strong className="text-white font-bold">{currentTier.rate} điểm CinePoint</strong> cho mỗi đơn vé. Thỏa sức <strong className="text-white font-bold">đổi vé 2D/3D miễn phí</strong> và nhận <strong className="text-white font-bold">quà bắp nước sinh nhật</strong> độc quyền.
                      </>
                    ) : (
                      <>
                        Hội viên NovaTicket nhận hoàn từ <strong className="text-white font-bold">5% đến 10% điểm CinePoint</strong> mỗi đơn hàng. Thỏa sức <strong className="text-white font-bold">đổi vé 2D/3D miễn phí</strong>, nhận <strong className="text-white font-bold">quà bắp nước sinh nhật</strong> và đặc quyền vào rạp qua <strong className="text-white font-bold">lối ưu tiên</strong>.
                      </>
                    )}
                  </p>
                  {!isAuthenticated ? (
                    <Link to="/auth/login" className="inline-flex items-center gap-1.5 text-xs font-bold text-[#F5C518] hover:underline mt-3">
                      <span>Đăng nhập để xem thẻ & điểm CinePoint của bạn</span>
                      <ChevronRight className="w-3.5 h-3.5" />
                    </Link>
                  ) : (
                    <div className="text-xs text-cinema-200 mt-3 flex items-center gap-2 font-mono font-semibold">
                      <span className="text-cinema-300">QUYỀN LỢI HIỆN TẠI:</span>
                      <span className={cn("font-bold px-2 py-0.5 rounded bg-white/10 border border-white/15", currentTier.titleColor)}>
              ...
              <div className="relative z-10 grid grid-cols-3 gap-3 pt-6 border-t border-white/10">
                <div className="bg-white/[0.04] p-3 rounded-xl border border-white/5 text-left">
                  <div className="font-mono text-base font-extrabold text-[#F5C518]">100%</div>
                  <div className="text-xs text-cinema-100 font-semibold mt-0.5">Đổi vé miễn phí</div>
                </div>
                <div className="bg-white/[0.04] p-3 rounded-xl border border-white/5 text-left">
                  <div className="font-mono text-base font-extrabold text-white">{currentTier.rate}</div>
                  <div className="text-xs text-cinema-100 font-semibold mt-0.5">Tích lũy điểm mỗi đơn</div>
                </div>
                <div className="bg-white/[0.04] p-3 rounded-xl border border-white/5 text-left">
                  <div className="font-mono text-base font-extrabold text-amber-400">BIRTHDAY</div>
                  <div className="text-xs text-cinema-100 font-semibold mt-0.5">Quà tặng sinh nhật</div>
                </div>
              </div>
            </div>

            {/* CARD 2: Fast Booking 60s */}
            <div className="relative overflow-hidden rounded-3xl bg-[#0e1722]/90 border border-white/10 p-7 sm:p-8 flex flex-col justify-between hover:border-white/20 transition-all duration-300 group hover:-translate-y-1">
              <div>
                <div className="flex items-center justify-between mb-4">
                  <span className="font-mono text-xs text-amber-300 font-extrabold tracking-wider">02 / SPEED TRACK</span>
                  <Zap className="w-6 h-6 text-[#F5C518] drop-shadow-[0_0_10px_rgba(245,197,24,0.4)]" />
                </div>
                <h3 className="font-display text-xl font-bold text-white mb-2.5 group-hover:text-[#F5C518] transition-colors">
                  Đặt Vé Siêu Tốc 60s
                </h3>
                <p className="text-cinema-200 text-xs leading-relaxed font-body">
                  Sơ đồ phòng chiếu <strong className="text-white">tương tác thời gian thực</strong>. Chọn chỗ ngồi trung tâm ưng ý, thanh toán 1-chạm và nhận vé điện tử <strong className="text-white">mã QR vào rạp tức thì</strong>.
                </p>
              </div>
              <div className="mt-6 pt-3.5 border-t border-white/5 bg-white/[0.04] p-3 rounded-xl border font-mono text-xs text-cinema-100 font-semibold flex items-center gap-2">
            ...
            <div className="relative overflow-hidden rounded-3xl bg-[#0e1722]/90 border border-white/10 p-7 sm:p-8 flex flex-col justify-between hover:border-white/20 transition-all duration-300 group hover:-translate-y-1">
              <div>
                <div className="flex items-center justify-between mb-4">
                  <span className="font-mono text-xs text-red-300 font-extrabold tracking-wider">03 / GOURMET COMBO</span>
                  <Popcorn className="w-6 h-6 text-[#ff6b6b] drop-shadow-[0_0_10px_rgba(229,9,20,0.4)]" />
                </div>
                <div className="flex items-center gap-2 mb-2">
                  <h3 className="font-display text-xl font-bold text-white group-hover:text-[#ff6b6b] transition-colors">
                ...
                <p className="text-cinema-200 text-xs leading-relaxed font-body">
                  Tiết kiệm đến <strong className="text-white">30%</strong> khi đặt trước Solo Combo hoặc Couple Combo kèm vé. Thưởng thức bắp rang bơ phô mai & caramel nóng hổi tại <strong className="text-white">quầy ưu tiên</strong>.
                </p>
              </div>
              <div className="mt-6 pt-3.5 border-t border-white/5 bg-white/[0.04] p-3 rounded-xl border font-mono text-xs text-cinema-100 font-semibold flex items-center gap-2">
            ...
            <div className="md:col-span-2 relative overflow-hidden rounded-3xl bg-[#0e1722]/90 border border-white/10 p-7 sm:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6 hover:border-white/20 transition-all duration-300 group hover:-translate-y-1">
              <div className="max-w-md">
                <div className="flex items-center gap-2 mb-2">
                  <span className="font-mono text-xs text-emerald-300 font-extrabold tracking-wider">04 / SECURE & FLEXIBLE</span>
                  <ShieldCheck className="w-5 h-5 text-emerald-400 drop-shadow-[0_0_10px_rgba(52,211,153,0.4)]" />
                </div>
                <h3 className="font-display text-xl sm:text-2xl font-bold text-white mb-2 group-hover:text-emerald-400 transition-colors">
                  Thanh Toán Bảo Mật Đa Tầng & Đổi Trả Linh Hoạt
                </h3>
                <p className="text-cinema-200 text-xs leading-relaxed font-body">
                  Mã hóa chuẩn <strong className="text-white">PCI-DSS</strong> với VNPay, thẻ ATM nội địa, Visa/Mastercard và QR Pay. Hỗ trợ <strong className="text-white">hủy vé trước giờ chiếu</strong> linh hoạt & minh bạch.
                </p>
              </div>
              <div className="shrink-0 flex items-center gap-2.5 flex-wrap">
                <div className="px-3.5 py-2 rounded-xl bg-white/[0.06] border border-white/10 font-mono text-xs text-cinema-100 font-semibold shadow-sm">
                  🛡️ 256-BIT SSL
                </div>
                <div className="px-3.5 py-2 rounded-xl bg-white/[0.06] border border-white/10 font-mono text-xs text-cinema-100 font-semibold shadow-sm">
                  💳 VNPAY · VISA · ATM
                </div>
                <div className="px-3.5 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/30 font-mono text-xs text-emerald-400 font-bold shadow-sm">
                  ✓ INSTANT REFUND
                </div>
              </div>
            </div>
