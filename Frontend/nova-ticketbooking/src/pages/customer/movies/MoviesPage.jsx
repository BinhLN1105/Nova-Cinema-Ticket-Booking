...
import { Search, Star, Clock, Ticket, X } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden" style={{ aspectRatio: "2/3" }}>
        ...
        />
        <div
          className="absolute inset-0 bg-gradient-to-t from-cinema-800/80 to-transparent
        ...
        {movie.status === "NOW_SHOWING" && (
          <span className="absolute top-2 right-2 badge badge-green text-xs">
            Đang chiếu
          </span>
        )}
        {movie.status === "COMING_SOON" && (
          <span className="absolute top-2 right-2 badge badge-blue text-xs">
            Sắp ra mắt
          </span>
        )}
        ...
        {movie.genres?.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {movie.genres.slice(0, 2).map((g) => (
              <span
                key={g.id}
                className="text-xs px-1.5 py-0.5 rounded bg-cinema-700 text-cinema-300"
              >
                {g.name}
              </span>
            ))}
