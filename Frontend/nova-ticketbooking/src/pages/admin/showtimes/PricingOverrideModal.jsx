...
export default function PricingOverrideModal({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      showtimeApi.overrideSeatPrices(
        showtimeId,
        selectedSeats,
        Number(newPrice),
      ),
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          <button
            onClick={onClose}
            className="absolute right-4 top-4 p-2 text-cinema-400 hover:text-white rounded-xl hover:bg-white/10 flex-shrink-0 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <p className="text-sm text-cinema-400 mb-6 shrink-0">
            Chọn các ghế cần đổi giá và nhập giá mới.
          </p>
                ...
                <div className="min-w-max mx-auto space-y-2 pb-6">
                  ...
                  {rows.map(({ rowLabel, seats }) => (
                    ...
                    >
                      <button
                        type="button"
                        className="w-6 text-center text-sm font-bold text-cinema-400 hover:text-white bg-transparent border-none p-0 cursor-pointer"
                        onClick={() => handleSelectAllRow(seats)}
                        title="Chọn cả hàng"
                        aria-label={`Chọn tất cả ghế hàng ${rowLabel}`}
                      >
                        {rowLabel}
                      </button>
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                isSelected
                                  ...
                                  : isBooked
                                    ? "bg-cinema-600 border-cinema-500 text-white/50 cursor-not-allowed"
                                    : isLocked
                                      ? "bg-yellow-500/50 border-yellow-500/50 text-white cursor-not-allowed"
                                      : seat.seatType === "VIP"
                                        ? "bg-yellow-500/10 border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/20"
                                        : seat.seatType === "COUPLE"
                                          ? "w-16 sm:w-[72px] bg-pink-500/10 border-pink-500/30 text-pink-400 hover:bg-pink-500/20"
                  ...
                  <div className="flex items-center justify-center gap-6 mt-8 p-4 bg-cinema-800 rounded-xl">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-cinema-300">
                        Đang chọn ({selectedSeats.length})
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 rounded bg-white/5 border-white/10" />
                      <span className="text-xs text-cinema-300">Thường</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 rounded bg-yellow-500/10 border-yellow-500/30" />
                      <span className="text-xs text-cinema-300">VIP</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-4 rounded bg-pink-500/10 border-pink-500/30" />
                      <span className="text-xs text-cinema-300">Couple</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 rounded bg-cinema-600 border-cinema-500" />
                      <span className="text-xs text-cinema-300">
                        Đã đặt/Khóa
                      </span>
                    </div>
                ...
                >
                  <div>
                    ...
                    {newPrice && (
                      <p className="text-xs text-brand-400 mt-2 font-medium">
                        Định mức: {formatCurrency(newPrice)}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="space-y-3 pt-6 border-t border-white/5">
                    ...
                    <button
                      type="submit"
                      disabled={
                        mutation.isPending || !selectedSeats.length || !newPrice
                      }
