...
import {
  ...
  Pagination,
  StatusBadge,
} from "@/components/common/ui/AdminTable";
import {
  SearchInput,
  Button,
  Select,
...
export default function UsersPage() {
  ...
  const assignCinemaMut = useMutation({
    mutationFn: () =>
      userApi.assignCinema(assignCinemaUser?.id, selectedCinema),
    onSuccess: () => {
    ...
    onError: (err) =>
      toast.error(err?.response?.data?.message || "Lỗi phân công rạp"),
  });
        ...
        return (
          <div className="flex items-center justify-end gap-1">
            ...
            {u.role === "STAFF" && (
              <button
                onClick={() => {
                  setAssignCinemaUser(u);
                  setSelectedCinema(u.cinemaId || "");
                }}
                className="p-2 rounded-lg hover:bg-brand-50 text-brand-400 hover:text-brand-600 transition-all font-medium text-xs whitespace-nowrap"
                title="Phân công rạp"
              >
            ...
            <button
              ...
              title={
                isSelf
                  ? "Bạn không thể tự đổi vai trò của mình"
                  : isAdmin
                    ? "Hệ thống bảo mật chặn thao tác trên tài khoản Admin"
                    : "Thay đổi vai trò"
              }
            >
            ...
            <button
              ...
              title={
                isSelf
                  ? "Bạn không thể tự khoá chính mình"
                  : isAdmin
                    ? "Hệ thống bảo mật chặn tài khoản Admin bị khoá"
                    : "Khoá tài khoản"
              }
            >
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <PageHeader
          ...
          subtitle={`${data?.totalElements ?? 0} tài khoản`}
        />
        <Button
          ...
          <div className="space-y-4">
            <div className="flex flex-col items-center gap-3 py-4">
              ...
              <div className="text-center">
                ...
                <div className="flex items-center justify-center gap-2 mt-1">
                  <StatusBadge
                    label={ROLE_BADGE[viewUser.role]?.label ?? viewUser.role}
            ...
            <div className="space-y-3 text-sm">
              {[
                ...
                {
                  label: "Ngày tham gia",
                  value: formatDate(viewUser.createdAt),
                  icon: "📅",
                },
      ...
      <Modal
        ...
        title="Tạo tài khoản Nhân viên (STAFF)"
        size="md"
      >
        <div className="space-y-4">
          <Input
          ...
          <Input
            ...
            type="email"
            placeholder="staff@novacinema.com"
          ...
          <div className="space-y-1.5">
            <label
              htmlFor="staffCinemaSelect"
              className="text-sm font-medium text-gray-700"
            >
              Phân công rạp (tùy chọn)
            </label>
            <Select
              id="staffCinemaSelect"
              placeholder="-- Chọn rạp --"
              value={staffForm.cinemaId}
              onChange={(e) =>
                setStaffForm((p) => ({ ...p, cinemaId: e.target.value }))
              }
          ...
          <div className="flex gap-3 pt-4 border-t">
            ...
            <Button
              ...
              disabled={
                !staffForm.fullName || !staffForm.email || !staffForm.password
              }
      ...
      {/* ── Assign Cinema Modal ── */}
      <Modal
        open={!!assignCinemaUser}
        onClose={() => setAssignCinemaUser(null)}
        title="Phân công rạp cho Nhân viên"
        size="sm"
      >
        {assignCinemaUser && (
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              Chọn rạp làm việc cho nhân viên{" "}
              <span className="font-bold text-gray-900">
                {assignCinemaUser.fullName}
              </span>
            </p>
            <Select
              placeholder="-- Chọn rạp --"
              ...
              options={[
                ...cinemaOptions,
                { value: "", label: "Không gắn rạp (Hủy phân công)" },
              ]}
            />
            ...
            <div className="flex gap-3 pt-4 border-t">
              <Button
                variant="ghost"
                className="flex-1"
                onClick={() => setAssignCinemaUser(null)}
              >
                Hủy
              </Button>
              <Button
                className="flex-1"
                loading={assignCinemaMut.isPending}
                onClick={() => assignCinemaMut.mutate()}
              >
                Cập nhật rạp
              </Button>
            </div>
