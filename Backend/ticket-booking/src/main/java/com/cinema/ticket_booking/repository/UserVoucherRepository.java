...
public interface UserVoucherRepository extends JpaRepository<UserVoucher, UUID> {
    ...
    @Query("SELECT uv FROM UserVoucher uv JOIN FETCH uv.voucher v WHERE uv.user.id = :userId AND uv.isUsed = false ORDER BY uv.savedAt DESC")
    List<UserVoucher> findUnusedByUserIdWithVoucher(UUID userId);
