...
public class ComboRepository {
    ...
    @Inject public ComboRepository(ApiService api) { this.api = api; }
    ...
    public LiveData<Resource<List<ComboResponse>>> getCombos() {
        ...
        api.getCombos().enqueue(new Callback<>() {
            @Override public void onResponse(Call<ApiResponse<List<ComboResponse>>> c, Response<ApiResponse<List<ComboResponse>>> res) {
                if (res.isSuccessful() && res.body() != null && res.body().isSuccess())
                    r.setValue(Resource.success(res.body().getData()));
                else r.setValue(Resource.error("Tải combo thất bại"));
            }
            @Override public void onFailure(Call<ApiResponse<List<ComboResponse>>> c, Throwable t) {
