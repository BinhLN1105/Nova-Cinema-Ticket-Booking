...
public class ComboAdapter extends RecyclerView.Adapter<ComboAdapter.VH> {
    public interface OnQtyChange { void onChange(String comboId); }
    ...
    public ComboAdapter(List<ComboResponse> items, OnQtyChange onAdd, OnQtyChange onRemove, Map<String, Integer> qtys) {
        this.items = items; this.onAdd = onAdd; this.onRemove = onRemove; this.qtys = qtys;
    }
    ...
    class VH extends RecyclerView.ViewHolder {
        ...
        VH(ItemComboBinding b) { super(b.getRoot()); this.b = b; }
        void bind(ComboResponse c) {
            ...
            b.btnMinus.setOnClickListener(v -> { onRemove.onChange(c.getId()); notifyItemChanged(getAdapterPosition()); });
            b.btnPlus.setOnClickListener(v -> { onAdd.onChange(c.getId()); notifyItemChanged(getAdapterPosition()); });
        }
    }
}
