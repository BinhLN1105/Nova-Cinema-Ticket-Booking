...
public class ShowtimeRepository {
    ...
    @Inject
    public ShowtimeRepository(ApiService api) {
        this.api = api;
    }
    ...
    public LiveData<Resource<List<ShowtimeResponse>>> getShowtimes(String movieId, String cinemaId, String date) {
        ...
        api.getShowtimes(movieId, cinemaId, date).enqueue(new Callback<>() {
            @Override
            public void onResponse(Call<ApiResponse<List<ShowtimeResponse>>> c,
                    Response<ApiResponse<List<ShowtimeResponse>>> res) {
                if (res.isSuccessful() && res.body() != null && res.body().isSuccess())
                    r.setValue(Resource.success(res.body().getData()));
                else
                    r.setValue(Resource.error("Tải suất chiếu thất bại"));
            }

            @Override
            public void onFailure(Call<ApiResponse<List<ShowtimeResponse>>> c, Throwable t) {
    ...
    public LiveData<Resource<SeatMapResponse>> getSeatMap(String showtimeId) {
        ...
        api.getSeatMap(showtimeId).enqueue(new Callback<>() {
            @Override
            public void onResponse(Call<ApiResponse<SeatMapResponse>> c, Response<ApiResponse<SeatMapResponse>> res) {
                if (res.isSuccessful() && res.body() != null && res.body().isSuccess())
                    r.setValue(Resource.success(res.body().getData()));
                else
                    r.setValue(Resource.error("Tải sơ đồ ghế thất bại"));
            }

            @Override
            public void onFailure(Call<ApiResponse<SeatMapResponse>> c, Throwable t) {
                r.setValue(Resource.error("Lỗi kết nối: " + t.getMessage()));
            }
        });
        return r;
