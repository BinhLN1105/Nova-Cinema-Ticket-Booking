...
public class UserRepository {
    ...
    public LiveData<Resource<UserResponse>> getProfile() {
        ...
        api.getMyProfile().enqueue(new Callback<>() {
            @Override public void onResponse(Call<ApiResponse<UserResponse>> c, Response<ApiResponse<UserResponse>> res) {
                if (res.isSuccessful() && res.body() != null && res.body().isSuccess())
                    r.setValue(Resource.success(res.body().getData()));
                else r.setValue(Resource.error("Tải profile thất bại"));
            }
            @Override public void onFailure(Call<ApiResponse<UserResponse>> c, Throwable t) {
