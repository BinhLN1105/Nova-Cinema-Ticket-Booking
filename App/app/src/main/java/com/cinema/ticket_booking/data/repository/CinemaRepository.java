...
public class CinemaRepository {
    ...
    @Inject
    public CinemaRepository(ApiService api) {
        this.api = api;
    }
    ...
    public LiveData<Resource<List<CinemaResponse>>> getCinemas(String city) {
        ...
        api.getCinemas(city).enqueue(new Callback<>() {
            @Override
            public void onResponse(Call<ApiResponse<List<CinemaResponse>>> c,
                    Response<ApiResponse<List<CinemaResponse>>> res) {
                if (res.isSuccessful() && res.body() != null && res.body().isSuccess())
                    r.setValue(Resource.success(res.body().getData()));
                else
                    r.setValue(Resource.error("Tải rạp thất bại"));
            }
